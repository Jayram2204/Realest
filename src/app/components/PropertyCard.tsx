...
export function PropertyCard({ property, onOpen, mineShares = 0 }: Props & { mineShares?: number }) {
  return (
    <article className="group border border-neutral-300 rounded-[2px] bg-white flex flex-col hover:border-neutral-900 transition-colors">
      ...
      <div className="px-4 py-3 border-b border-neutral-300">
        <div className="flex items-start gap-3">
          <Identicon value={property.mintTxHash} size={36} />
      ...
      <dl className="grid grid-cols-2 bg-indigo-50/40">
        ...
        <Stat label="Holders" value={`${Math.max(1, property.totalShares - property.availableShares > 0 ? 147 : 0)}`} top />
      </dl>

      <div className="px-4 py-3 border-t border-neutral-300">
        <OwnershipBar
          total={property.totalShares}
          sold={property.totalShares - property.availableShares}
          mine={mineShares}
          height={8}
        />
      </div>
